...
// Moved from `diagram/DiagramTraceModel.kt` during the sequence-diagram v3 cutover (phase 6 of
// docs/plans/use-the-claude-design-mcp-compiled-lighthouse.md): `source/SourceTraceInference.kt`
// depends on these types, and v3 (`com.indagium.diagram3`) does no source-trace enrichment, so this
// is the one file from the old `diagram` package that had to survive its deletion. Package renamed
// from `com.indagium.diagram` to `com.indagium.source`; type names are unchanged from the original
// so every existing caller only had to update its import.
 ...
 * transient; only this bounded user choice is persisted. Moved from `diagram/DiagramModel.kt`
 * alongside the rest of this file for the same reason — [com.indagium.source.SourceTraceInference]
 * and the `build_sequence_diagram` MCP handler are its only callers now that `diagram/` is gone. */
