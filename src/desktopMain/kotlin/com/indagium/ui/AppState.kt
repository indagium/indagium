...
import com.indagium.ai.AiContextRequest
import com.indagium.ai.AiEvidence
import com.indagium.ai.AiInvestigationContext
import com.indagium.ai.AiPromptRequest
import com.indagium.ai.AiQuickAction
import com.indagium.ai.AiSessionRegistry
...
enum class FollowMappingStatus {
     ...
     *  [com.indagium.utils.passesFilter], not just inferred from a floor mismatch), so Follow is
...
class AppState(
    ...
    internal val aiSidebarRuntime = AiSidebarRuntime(
        sessions = aiSessions,
        toolGatewayFactory = { IndagiumToolOperations(this).toolGateway },
        maxToolRounds = { settings.aiMaxToolRounds },
        accountAgentRunnerFactory = {
            com.indagium.ai.AccountAgentRunner(
                managedMcpServerFactory = { run -> com.indagium.ai.ManagedMcpServerLease.start(this, run) },
                maxToolRounds = settings.aiMaxToolRounds,
            )
        },
    )
    ...
    fun updateAiProviderProfile(profile: AiProviderProfile): String? {
        ...
        if (!validation.isValid) {
            if (current.baseUrl.trim() != profile.baseUrl.trim() &&
                validation.problem == com.indagium.ai.AiProviderUrlProblem.REMOTE_DISCLOSURE_REQUIRED
            ) {
                updateSettings {
    ...
    // IndagiumToolOperations.awaitLoad) that triggered one specific tab's load must not block on —
    ...
    // polling to (ARCH-3: IndagiumToolOperations) doesn't have to re-derive it from sourcePath
    ...
    // Best-effort app/build version fill-in at save time (com.indagium.cases' "similar past
    ...
    // only inside debug/IndagiumToolOperations, as it used to) so the UI dialog and the AI/MCP tool
    ...
    // `private`) so IndagiumToolOperations and tests can reach it.
