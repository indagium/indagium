 ...
 * is user-downloaded. Run with OPENLOG_VOICE_MODEL and OPENLOG_VOICE_FIXTURE environment variables
 * to prove that the installed model and JNI binding recognize a known-good audio file.
 */
class WhisperJniManualIntegrationTest {
    @Test
    fun recognizesKnownGoodFixtureWhenConfigured() {
        val model = System.getenv("OPENLOG_VOICE_MODEL")?.let(::File) ?: return
        val fixture = System.getenv("OPENLOG_VOICE_FIXTURE")?.let(::File) ?: return
        require(model.isFile) { "OPENLOG_VOICE_MODEL must point to an installed Whisper model" }
        require(fixture.isFile) { "OPENLOG_VOICE_FIXTURE must point to an audio fixture" }
