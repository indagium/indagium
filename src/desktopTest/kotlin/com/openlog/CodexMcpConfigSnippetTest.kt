...
class CodexMcpConfigSnippetTest {
    ...
    fun usesTomlWithAStaticBearerHeader() {
        ...
        assertTrue(snippet.contains("[mcp_servers.openlog]"), "Codex config must define the openLog server:\n$snippet")
    ...
    fun approvesOnlyListTabs() {
        ...
        assertTrue(snippet.contains("[mcp_servers.openlog.tools.list_tabs]"), "Only list_tabs should be pre-approved:\n$snippet")
        assertTrue(snippet.contains("approval_mode = \"approve\""), "list_tabs should run in non-interactive Codex calls:\n$snippet")
        assertTrue(!snippet.contains("default_tools_approval_mode"), "Other openLog tools must keep Codex's normal approval flow:\n$snippet")
    }
}
