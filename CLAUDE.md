...
| `source` | Source indexing and log→call-site resolution. Pure text/regex/brace-matching, no compiler dep. Own store at `appDataDir()/source-index` (`indagium-source-index-v1`, schema v10; also accepts the legacy `openLog2-source-index-v1` magic). |
