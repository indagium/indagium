...
| `debug` | `ControlServer` (Ktor CIO, loopback-only, MCP over Streamable HTTP + REST, off by default), the **75-tool** catalogue + handlers joined by `IndagiumToolGateway`, hand-rolled `Json`, `AppLogger`. |
